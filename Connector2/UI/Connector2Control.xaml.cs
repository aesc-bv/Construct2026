...
using ConnectorModel = AESCConstruct2026.Connector2.Connector2;
...
namespace AESCConstruct2026.Connector2.UI
{
    public partial class Connector2Control : UserControl, Localization.ILocalizable
    {
        ...
        public Connector2Control()
        {
            try
            {
                ...
                this.Loaded += Connector2Control_Loaded;
                this.Unloaded += Connector2Control_Unloaded;
        ...
        // ILocalizable: RelocalizeAll already walked the Tag tree, so only the
        // non-Tag tooltips need re-applying when the language changes live.
        ...
        private void LocalizeTooltips()
        {
            ...
            {
                Logger.Log("Connector2Control LocalizeTooltips failed: " + ex.ToString());
            }
        }
        ...
        // Connector2Control_Loaded runs once when the control is loaded to wire UI events, load presets, and draw the initial connector.
        private void Connector2Control_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
            ...
            catch (Exception ex) { Logger.Log("Connector2Control_Loaded failed: " + ex.ToString()); }
        }
        ...
        // Connector2Control_Unloaded unsubscribes from viewport events and clears any 3D preview bodies.
        private void Connector2Control_Unloaded(object sender, RoutedEventArgs e)
        {
            try
            {
                var win = Window.ActiveWindow;
            ...
            catch (Exception ex) { Logger.Log("Connector2Control_Unloaded unsub: " + ex.ToString()); }
        ...
        private static double? GetDouble(Dictionary<string, int> headers, string[] cols, string key)
        {
            ...
            if (NumberParsing.TryParseUserInput(cols[i], out var v)) return v;
            return null;
        }
        ...
        // Parses a double from a TextBox, accepting both '.' and ',' decimal marks regardless of locale.
        private bool TryReadDouble(TextBox tb, out double v)
        {
            if (tb == null) { v = 0; return false; }
        ...
        private void EnforceRectCutExclusivity()
        {
            ...
            if (connectorSpacing != null && NumberParsing.TryParseUserInput(connectorSpacing.Text, out var rel) && rel > 0)
            {
        ...
        private void EnforceCornerCoupling()
        {
            ...
            if (NumberParsing.TryParseUserInput(connectorRadiusChamfer?.Text, out var rc) && rc > 0)
            {
