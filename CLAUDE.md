...
- **Localization** (`UIMain/Language.cs`): CSV-based translation. The **canonical source is `Language/languageConstruct.csv` in the repo** (semicolon-delimited, Windows-1252, columns `ID;EN;NL;DE;FR;IT;ES`). A post-build MSBuild target (`SyncLanguageCsv`) copies it to the runtime path `%ProgramData%\AESCConstruct\Language\languageConstruct.csv`, and the installer ships it from the repo. **Edit the repo file, never ProgramData directly.** Access keys via `Language.Translate("key")`, or the call-site helper `L.T(key)` / `L.F(key, args)` / `L.Status(key, type, args)` (`UIMain/L.cs`). Controls with non-Tag strings (tooltips, watermarks, grid headers) implement `Localization.ILocalizable` so `UIManager.RelocalizeAll()` refreshes them on live language change. New keys are recorded in `_translation_staging.txt`. Use ASCII `->` (not the Unicode arrow) — it is not Windows-1252-representable.
