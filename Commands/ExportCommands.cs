...
namespace AESCConstruct25.Utilities
{
    ...
    {
        public static void ExportBOM(Window window)
        {
            ...
            try
            {
                var sheet = window.Scene as DrawingSheet;
                if (sheet == null)
                {
                    MessageBox.Show(
                ...
                double width = 0.30;
                double columnWidth = width / cols; // e.g. 300mm total width
                var origin = Point.Create(sheet.Width - width - margin, rowHeight * bomRows.Count + margin * 3, 0);
                var anchorUV = new PointUV(origin.X, origin.Y);
        ...
        public static (double aStart, double aEnd) GetProfileCutAngles(Component comp)
        {
            ...
            return (180.0 - Measure(n0), 180.0 - Measure(n1));
        }
