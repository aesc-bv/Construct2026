...
- **Symptom:** when `OffsetFaces` throws, the catch only logs. The cut body is then used
  **un-inflated**, so the subtracted cavity is exactly the connector size — zero clearance.
  The user gets a tight/interfering joint with no warning; only a log line records it.
...
### P3-6 · `AreCoaxial` parallel/cos-angle math is effectively dead
- **Where:** `Connector2/CylInfo2.cs:95-99` — only `a.IsCoincident(b)` is used; the
  documented angle-tolerance / dot-product path doesn't gate the result.
- **Fix:** remove the dead math (and the misleading `angTol` doc) or actually use it for a
  parallel-but-offset tolerance. Effort: S.
...
1. **P1-1** (wrong geometry, highest user impact) → **P1-3** → **P1-2**.
2. **P2-2** and **P2-1** (dynamic height / cylindrical preview — most-noticed gaps).
3. **P2-3 / P2-4 / P2-5** robustness.
4. P3 sweep (mostly mechanical; bundle into one cleanup commit).
